...
It's not stated explicility, but the Sprint Retrospective is also the right time to discuss changing the Sprint length if that's desired. That way if a change is agreed upon, the change can be applied to the next Sprint that starts when the retrospective ends.
...
And yes, I do realize this is probably the fourth or fifth time I've mentioned the duration of each of the Scrum Events in this book. That's because you're likely going to get five or six questions on the topic, so it's not a topic to treat trivally.
